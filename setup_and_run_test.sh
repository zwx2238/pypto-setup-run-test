...
generate_env_file() {
    ...
    log_info "Generating env file: ${ENV_FILE}"
    {
        ...
        printf '# Usage from Python: benchmark.py loads this automatically.\n\n'
        ...
        local llm_var
        for llm_var in AKG_AGENTS_API_KEY AKG_AGENTS_BASE_URL AKG_AGENTS_MODEL_NAME; do
            if [[ -n "${!llm_var:-}" ]]; then
                printf '%s=%s\n' "${llm_var}" "${!llm_var}"
            fi
        done
...
check_llm_config() {
    ...
    local missing=()
    [[ -n "${AKG_AGENTS_API_KEY:-}" ]]    || missing+=("AKG_AGENTS_API_KEY")
    [[ -n "${AKG_AGENTS_BASE_URL:-}" ]]   || missing+=("AKG_AGENTS_BASE_URL")
    [[ -n "${AKG_AGENTS_MODEL_NAME:-}" ]] || missing+=("AKG_AGENTS_MODEL_NAME")

    if [[ ${#missing[@]} -gt 0 ]]; then
        die "Missing required environment variables: ${missing[*]}. Export them before running this script."
    fi

    log_info "AKG_AGENTS_API_KEY=sk-****"
    log_info "AKG_AGENTS_BASE_URL=${AKG_AGENTS_BASE_URL}"
    log_info "AKG_AGENTS_MODEL_NAME=${AKG_AGENTS_MODEL_NAME}"
}
